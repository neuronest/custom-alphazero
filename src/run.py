...
def visualize_mcts_iteration(
    ...
    assert mcts_name.endswith(f"iteration_{iteration}")
    mcts_visualizer = MctsVisualizer(mcts_tree.root, mcts_name=mcts_name)
    ...
    if "updated" in mcts_name:
        assert run_id is not None
        mcts_visualizer.save_as_pdf(
            directory=os.path.join(ConfigPath.results_path, ConfigGeneral.game, run_id)
        )
    ...
    for _ in range(ConfigGeneral.training_iterations):
        ...
        if (
            ...
            visualize_mcts_iteration(
                mcts_tree, mcts_name, iteration, iteration_path, run_id=run_id
            )
            # if model has been updated it will participate in the next self-play phase and in next mcts tree
            mcts_name = (
