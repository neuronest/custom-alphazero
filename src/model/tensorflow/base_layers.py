...
class OuterConvBlock(Layer):
    def __init__(
        ...
        if self.residual_connexion:
            self.residual_connexion_layer = InnerConvBlock(
                input_dim=input_dim,
                filters=filters,
                kernel_size=(1, 1),
                strides=(1, 1),
            ...
            self.residual_connexion_layer, self.add_residual = None, None
    ...
    def call(self, inputs, training=False):
        ...
        if self.residual_connexion:
            identity_resized = self.residual_connexion_layer(inputs)
